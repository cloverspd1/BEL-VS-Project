...
function BindValidation() {
    ...
    $(".alphanumericwithspace").off("keypress").on("keypress", function (evt) {
        return isAlphaNumericWithSpace(evt, this);
    });
...
function BindDatePicker(selector) {
    ...
    $(selector + '.datepicker').each(function () {
        ...
        $(this).datetimepicker({
            ...
            minDate: $(this).hasClass("pastDisabled") ? new Date(todayDate.getFullYear(), todayDate.getMonth(), todayDate.getDate() + 7, 00, 00, 00) : todayDate.addDays(7)
...
Date.prototype.addDays = function (days) {
    ...
    endDate = new Date(todayDate.setDate(todayDate.getDate() + parseInt(days)));
    //while (count < noOfDaysToAdd) {
    //    endDate = new Date(todayDate.setDate(todayDate.getDate() + 1));
...
function ConfirmSubmit(ele) {
    ...
    if (isValid) {
        ...
        if ($(".field-validation-error:first").length > 0) {
            $("html,body").animate({ "scrollTop": $(".field-validation-error:first").offset().top - 200 });
            //$("html,body").animate({ "scrollTop": $(".field-validation-error:first").parents(".card:first").offset().top - 100 });
            setTimeout(function () {
                $(".field-validation-error:first").parent().find("select,input,textarea").first().focus();
            }, 10);
...
function Print(ele) {
    ...
    frameDoc.document.write("<html><head><style type='text/css'> .table{font-size:13px !important; padding:1px !important;}" +
...
function isAlphaNumericWithSpace(evt, control) {
