            ...
            if (context != null && web != null)
            {
                ...
                foreach (PropertyInfo property in propertyInfo)
                {
                    ...
                    if (isListColumn)
                    {
                        ...
                        {
                            selectClause = selectClause.Trim(',') + "," + propertyName + "/ID" + "," + propertyName + "/Title";
                            expandClause = expandClause.Trim(',') + "," + propertyName;
                        }
                        else if (isFile)
                        {
            ...
            foreach (PropertyInfo property in propertyInfo)
            {
                ...
                if (isListColumn)
                {
                    ...
                    else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && !property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                    {
                        ...
                        if (property.GetCustomAttribute<IsPersonAttribute>().IsMultiple)
                        {
                            if (((JContainer)item[listCoumnName])["results"] != null)
                            {
                                ...
                                {
                                    personEmail = personEmail.Trim(',') + "," + person["ID"].ToString();
                                }
                            }
                        }
                        else
                        {
                            ...
                            {
                                personEmail = ((dynamic)item[listCoumnName])["ID"].ToString();
                            }
                        }
                    ...
                    else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                    {
                        ...
                        property.SetValue(approver, personName.Trim(','));
                    }
                    else if (isFile)
                    {
        ...
        public bool SaveApproverMatrix(ClientContext context, Web web, List<ApplicationStatus> approverMatrixList, string listName) // Jatin 
        {
            if (context != null && web != null && approverMatrixList != null)
            {
                ...
                foreach (ApplicationStatus approver in approverMatrixList)
                {
                    ...
                    {
                        Logger.Error("Error While  SaveApproverMatrix");
                        Logger.Error(ex);
                    }
                }
            }
                    ...
                    if (approvers != null && approvers.Count != 0 && !string.IsNullOrEmpty(userEmail))
                    {
                        ...
                        switch (actionPerformed)
                        {
                            ...
                            case ButtonActionStatus.NextApproval:
                                if (approvers.Any(p => !string.IsNullOrEmpty(p.Approver) && p.Levels == currLevel.ToString() && p.Approver.Contains(userEmail)))
                                {
                                    approvers.FirstOrDefault(p => !string.IsNullOrEmpty(p.Approver) && p.Levels == currLevel.ToString() && p.Approver.Contains(userEmail)).Status = ApproverStatus.APPROVED;
                                }
                                ...
                                approvers.ForEach(p =>
                                {
                                    if (!string.IsNullOrEmpty(p.Approver) && p.Levels == currLevel.ToString() && p.Approver.Contains(userEmail))
                                    {
